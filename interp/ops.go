...
func callBuiltin(caller *frame, callpos token.Pos, fn *ssa.Builtin, args []value, argSrc []string) (value, []string) {
	switch fn.Name() {
	case "append":
		if len(args) == 1 {
			return args[0], []string{argSrc[0]}
		}
		if s, ok := args[1].(string); ok {
			...
			return arg0, []string{fmt.Sprintf("[%v %v]", argSrc[0], argSrc[1])}
		}
		// append([]T, ...[]T) []T
		return append(args[0].([]value), args[1].([]value)...), []string{fmt.Sprintf("[%v, %v]", argSrc[0], argSrc[1])}

	case "copy": // copy([]T, []T) int
		if _, ok := args[1].(string); ok {
			panic("copy([]byte, string) not yet implemented")
		}
        r := copy(args[0].([]value), args[1].([]value))
        return r, []string{fmt.Sprintf("%v", r)}
	...
	case "len":
		...
		case string:
			return len(x), []string{fmt.Sprintf("%v", len(x))}
		case array:
			return len(x), []string{fmt.Sprintf("%v", len(x))}
		case *value:
			return len((*x).(array)), []string{fmt.Sprintf("%v", len((*x).(array)))}
		case []value:
			return len(x), []string{fmt.Sprintf("%v", len(x))}
		case map[value]value:
			return len(x), []string{fmt.Sprintf("%v", len(x))}
		case *hashmap:
			return x.len(), []string{fmt.Sprintf("%v", x.len())}
		case chan value:
			return len(x), []string{fmt.Sprintf("%v", len(x))}
		default:
	...
	case "cap":
		switch x := args[0].(type) {
		case array:
			return cap(x), []string{fmt.Sprintf("%v", cap(x))}
		case *value:
			return cap((*x).(array)), []string{fmt.Sprintf("%v", cap((*x).(array)))}
		case []value:
			return cap(x), []string{fmt.Sprintf("%v", cap(x))}
		case chan value:
			return cap(x), []string{fmt.Sprintf("%v", cap(x))}
	...
	case "real":
		...
		case complex64:
			return real(c), []string{fmt.Sprintf("%v", real(c))}
		case complex128:
			return real(c), []string{fmt.Sprintf("%v", real(c))}
		default:
	...
	case "imag":
		switch c := args[0].(type) {
		case complex64:
			return imag(c), []string{fmt.Sprintf("%v", imag(c))}
		case complex128:
			return imag(c), []string{fmt.Sprintf("%v", imag(c))}
		default:
	...
	case "complex":
		...
		case float32:
            r := complex(f, args[1].(float32))
			return r, []string{fmt.Sprintf("%v", r)}
		case float64:
            r := complex(f, args[1].(float64))
            return r, []string{fmt.Sprintf("%v", r)}
		default:
	...
	case "recover":
		...
		if caller.i.mode&DisableRecover == 0 &&
			...
			switch p := p.(type) {
			case targetPanic:
				return p.v, []string{fmt.Sprintf("%v", p.v)}
			case runtime.Error:
				// TODO(adonovan): must box this up
				// inside instance of interface 'error'.
				return iface{types.Typ[types.String], p.Error()}, []string{""}
			case string:
				return iface{types.Typ[types.String], p}, []string{""}
			default:
